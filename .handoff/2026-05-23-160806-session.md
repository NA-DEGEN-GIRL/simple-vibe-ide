...
## Metadata
- Created at: 2026-05-23 16:08:06 +0900
...
- Done:
  ...
  - Latest patch: Terminal widget pane index. Terminal widget tab rendering, close-widget, active-pane sync, and widget fit scheduling now use a widget-to-pane index instead of scanning the full terminal list for each widget operation.
  ...
  - `codex.md` has privacy-safe patch notes through the Terminal widget pane index patch.
- In progress:
...
## Files Touched
- `src/main.ts`: main frontend performance work, including latest terminal widget pane index plus prior Browser console/forward trims.
...
- Preserve existing terminal, Browser, Explorer, Editor, Image, Notes, forwarding, profile, export, and workspace behavior while reducing repeated scans, DOM queries, same-order set/prune allocations, `innerHTML` rebuilds, object/array churn, large synchronous writes, unbounded console formatting/serialization, and all-terminal scans for per-widget terminal work.
...
2. Smoke test terminal widgets: create many widgets/tabs, close active/inactive tabs, close whole widgets, switch active prompt target, resize/fit widgets, hide/show workspaces, and restore/switch workspaces.
3. Smoke test Browser console behavior: visible huge string/array/object/nested/circular-like logs, normal small object detail, console trimming, hidden replay, Browser panel hide/show, and local-port auto-forward scanning.
4. Smoke test Browser forward/detected-port behavior: detected-port rows, manual forwards, auto-forwarded Browser opens, stop/ignore actions, proxy-local-port filtering, hidden/visible Browser panel transitions, and workspace switching.
5. Smoke test Browser frame/many-tab behavior under noisy logs and hidden/visible panel changes.
6. Smoke test Explorer large-directory scrolling, selection, typeahead, rename/delete, file-size toggle, expanded/collapsed trees, cache reuse/expiry, and workspace switching.
...
- Command: `npm run build`
  - Result: pass; main app chunk 275.03 kB minified, CSS 40.13 kB minified, `xterm` lazy chunk 289.71 kB minified.
...
- Whether every optimized same-order/direct-DOM/cache/index/bounded-console/terminal-pane-index path preserves edge-case runtime behavior under heavy real interaction.
