...
## Metadata
- Created at: 2026-05-23 16:27:50 +0900
...
- Done:
  ...
  - Latest code patch: Clipboard image paste scan trim. Clipboard image detection now scans `DataTransferItemList`, `FileList`, and type lists directly instead of allocating temporary spread arrays and using `find`/`some`.
  ...
  - `codex.md` has privacy-safe patch notes through the clipboard scan trim and Windows smoke handoff patch.
- In progress:
...
## Files Touched
- `src/main.ts`: main frontend performance work, including latest clipboard image paste scan trim plus prior terminal widget pane index and Browser console/forward trims.
...
- Preserve existing terminal, Browser, Explorer, Editor, Image, Notes, forwarding, profile, export, clipboard image paste, and workspace behavior while reducing repeated scans, DOM queries, same-order set/prune allocations, `innerHTML` rebuilds, object/array churn, large synchronous writes, unbounded console formatting/serialization, all-terminal scans for per-widget terminal work, and clipboard array allocation.
...
3. Smoke test terminal widgets: create many widgets/tabs, close active/inactive tabs, close whole widgets, switch active prompt target, resize/fit widgets, hide/show workspaces, and restore/switch workspaces.
4. Smoke test Browser console behavior: visible huge string/array/object/nested/circular-like logs, normal small object detail, console trimming, hidden replay, Browser panel hide/show, and local-port auto-forward scanning.
...
7. Smoke test image clipboard paste: browser/source screenshots, multiple files in clipboard, image history update, image preview, and paste-tag-to-terminal.
8. If lag remains, profile the real Windows runtime to separate Explorer DOM/cache/backend polling, terminal write/scan/fit/layout, Browser iframe/console/frame/proxy/forward, workspace persistence, clipboard image processing, and tab/control render work.
...
- Command: `npm run build`
  - Result: pass; main app chunk 275.35 kB minified, CSS 40.13 kB minified, `xterm` lazy chunk 289.71 kB minified.
...
- Whether every optimized same-order/direct-DOM/cache/index/bounded-console/terminal-pane-index/clipboard-scan path preserves edge-case runtime behavior under heavy real interaction.
