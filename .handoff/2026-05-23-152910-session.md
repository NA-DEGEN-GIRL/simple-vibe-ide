...
## Metadata
- Created at: 2026-05-23 15:29:10 +0900
...
- Done:
  ...
  - Latest patch: Browser console formatter bounding. Visible console formatting truncates huge strings earlier and summarizes large arrays/wide objects before `JSON.stringify`, reducing UI stalls from noisy preview-app logs while keeping normal small console values detailed.
  ...
  - `codex.md` has privacy-safe patch notes through the Browser console formatter bounding patch.
- In progress:
...
## Files Touched
- `src/main.ts`: main frontend performance work, including latest Browser console formatter bounding and Browser forward row allocation trim.
...
- Preserve existing terminal, Browser, Explorer, Editor, Image, Notes, forwarding, profile, export, and workspace behavior while reducing repeated scans, DOM queries, same-order set/prune allocations, `innerHTML` rebuilds, object/array churn, large synchronous writes, and unbounded console formatting.
- Keep normal Browser console detail for small values, but summarize very large arrays/wide objects before expensive serialization to avoid preview log stalls.
...
2. Smoke test Browser console behavior: visible huge string/array/object logs, normal small object detail, console trimming, hidden replay, Browser panel hide/show, and local-port auto-forward scanning.
...
- Command: `npm run build`
  - Result: pass; main app chunk 274.23 kB minified, CSS 40.13 kB minified, `xterm` lazy chunk 289.71 kB minified.
...
- Whether every optimized same-order/direct-DOM/cache/index/console-bounding path preserves edge-case runtime behavior under heavy real interaction.
