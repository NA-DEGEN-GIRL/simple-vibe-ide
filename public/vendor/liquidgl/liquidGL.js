...
(() => {
  ...
  function resolveOverlayContainer(value) {
  ...
  class liquidGLRenderer {
    ...
    async _captureSnapshotOnce() {
      ...
      const attemptCapture = async (
        ...
        try {
          ...
          // Simple Vibe IDE keeps liquidGL canvases marked with data-liquid-ignore
          // and excludes them via ignoreElements below. Hiding the live canvas during
          // every recapture causes a visible white/CSS fallback flash when a window is
          // restored or many glass scopes recapture in sequence, so keep the previous
          // liquid frame visible while html2canvas captures the new background.
    ...
    _uploadTexture(srcCanvas) {
      ...
      const sameSize =
        srcCanvas.width === this.textureWidth &&
        srcCanvas.height === this.textureHeight;
      if (sameSize) {
        gl.texSubImage2D(
          gl.TEXTURE_2D,
          0,
    ...
    render() {
      ...
      this._updateDynamicNodes(snapshotRect);
      ...
      this.lenses.forEach((lens) => {
        ...
        this._renderLens(lens, snapshotRect);
        if (typeof lens._sviAfterRenderLens === "function") {
          try {
            lens._sviAfterRenderLens(this, snapshotRect);
          } catch (error) {
            if (!lens._sviAfterRenderLensWarned) {
              console.warn("[liquidGL] lens post-render hook failed", error);
              lens._sviAfterRenderLensWarned = true;
            }
          }
        }
      });
    ...
    _renderLens(lens, snapshotRect) {
      ...
      const snapRect =
        snapshotRect || this.snapshotTarget.getBoundingClientRect();
    ...
    /* ----------------------------- */
    _updateDynamicNodes(snapshotRect) {
      if (this._isScrolling && this._scrollUpdateCounter % 2 !== 0) return;
      const gl = this.gl;
      if (
        !this.texture ||
  ...
  class liquidGLLens {
    constructor(renderer, element, options) {
      ...
      this._preserveTargetOpacity = this.options.preserveTargetOpacity === true;
      this._preservePointerEvents = this.options.preservePointerEvents === true;
      this.el.style.transition = "none";
      this.el.style.opacity = this._preserveTargetOpacity
      ...
      if (!this._preservePointerEvents) {
    ...
    setShadow(enabled) {
      ...
      if (enabled) {
        ...
        if (!this._shadowEl) {
          ...
          resolveOverlayContainer(this.options.overlayContainer).appendChild(
            this._shadowEl
          );
    ...
    /* ----------------------------- */
    _reveal() {
      if (this._preserveTargetOpacity) {
      ...
      const animate = () => {
        ...
        this.renderer.lenses.forEach((ln) => {
          ...
          if (ln._preserveTargetOpacity) {
            ln.el.style.opacity = ln.originalOpacity || 1;
          } else {
    ...
    _bindTiltHandlers() {
      ...
      this._boundTouchEnd = this._boundTouchEnd || this._onTouchEnd.bind(this);
      ...
      this.el.addEventListener("touchstart", this._boundTouchStart, {
        passive: true,
      });
      this.el.addEventListener("touchmove", this._boundTouchMove, {
        passive: true,
      });
      this.el.addEventListener("touchend", this._boundTouchEnd, {
        passive: true,
      });
    ...
    _unbindTiltHandlers() {
      ...
      if (this._boundTouchStart) {
        this.el.removeEventListener("touchstart", this._boundTouchStart);
      }
      if (this._boundTouchMove) {
        this.el.removeEventListener("touchmove", this._boundTouchMove);
      }
      if (this._boundTouchEnd) {
        this.el.removeEventListener("touchend", this._boundTouchEnd);
      }
